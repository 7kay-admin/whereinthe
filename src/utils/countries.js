...
export function getCountriesByContinent(givenContinentCode) {
    ...
    return continentCountries.map((x) => ({ code: x[0].toLowerCase(), ...x[1]})).sort((a, b) => a.name.localeCompare(b.name))
...
export function getCountryInContinentIndex(givenCountryCode, givenContinentCode) {
    return getCountriesByContinent(givenContinentCode).sort((a, b) => a.name.localeCompare(b.name)).findIndex((x) => x.code === givenCountryCode);
}

export function getCountryFromContinentByIndex(givenIndex, givenContinentCode) {
    const alphabetisedCountriesInContinent = getCountriesByContinent(givenContinentCode).sort((a, b) => a.name.localeCompare(b.name));
    ...
    console.log('sanitisedIndex', sanitisedIndex, alphabetisedCountriesInContinent.length)

    return alphabetisedCountriesInContinent.filter((_, index) => sanitisedIndex === index)[0] || {};
}